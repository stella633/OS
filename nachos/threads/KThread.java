...
public class KThread {
     ...
     */
    public void fork() {
	Lib.assertTrue(status == statusNew);//현재 상태가 New인지 확인
	Lib.assertTrue(target != null);//running상태가 아닌지 확인
	
	Lib.debug(dbgThread,
		  "Forking thread: " + toString() + " Runnable: " + target);

	boolean intStatus = Machine.interrupt().disable();//인터럽트 disable()
	...
	while ((waitThread = currentThread.waitJoin.nextThread()) != null) {
		waitThread.ready();
	}
	sleep();
    }
	...
	if (status != statusFinished) {
		waitJoin.waitForAccess(currentThread);
		KThread.sleep();
	}
...
*/
    ...
    ThreadQueue waitJoin = ThreadedKernel.scheduler.newThreadQueue(true);
}
